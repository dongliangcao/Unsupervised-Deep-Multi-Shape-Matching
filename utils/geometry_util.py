...
def get_operators(verts, faces, k=120, normals=None,
    ...
    if not found:
        print('recompute the spectral operators')
        # recompute
...
def random_rotate_points(pts):
    """
    Random rotate points
    Args:
        pts (torch.Tensor): points [B, V, 3] or [V, 3]
    """
    assert pts.dim() == 2 or pts.dim() == 3
...
def random_rotation_matrix():
    ...
    theta = 2.0 * np.pi * theta # rotation around the pole.
    phi = 2.0 * np.pi * theta # for the direction of pole deflection.
    z = 2.0 * z # for magnitude of pole deflection
    ...
    # compute the rotation matrix (VV' - I)R
