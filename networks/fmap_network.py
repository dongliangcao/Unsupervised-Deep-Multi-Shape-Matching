# The FM solvers are adapted from https://github.com/pvnieo/DPFM and https://github.com/pvnieo/SURFMNet-pytorch
...
class RegularizedFMNet(nn.Module):
    ...
    def forward(self, feat_x, feat_y, evals_x, evals_y, evecs_trans_x, evecs_trans_y):
        ...
        D = self._get_mask(evals_x.flatten(), evals_y.flatten()).unsqueeze(0)  # [1, K, K]
    ...
    def _get_mask(self, evals1, evals2):
        scaling_factor = max(torch.max(evals1), torch.max(evals2))
        evals1, evals2 = evals1 / scaling_factor, evals2 / scaling_factor
        evals_gamma1 = (evals1 ** self.resolvant_gamma)[None, :]
        evals_gamma2 = (evals2 ** self.resolvant_gamma)[:, None]
