...
from utils.geometry_util import get_all_operators, to_basis, from_basis, compute_hks_autoscale, random_rotate_points
...
class DiffusionNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        for i_block in range(self.n_block):
            ...
            self.blocks += [block]
            self.add_module(f'block_{i_block}', self.blocks[-1])
    ...
    def forward(self, verts, faces, feats=None):
        ...
        Args:
            data (dict): data dict.
        """
        assert verts.dim() == 3 and faces.dim() == 3, 'Only support batch operation'

        # ensure reproducibility to first convert to cpu to find the precomputed spectral ops
        _, mass, L, evals, evecs, gradX, gradY = get_all_operators(verts.cpu(), faces.cpu(), k=self.k_eig, cache_dir=self.cache_dir)
